  ...
  class OpKernelImpl final : public OpKernel {
   public:
    ...
    Status Compute(OpKernelContext* ctx) const override {
      ...
      auto builder = output->RepBuilder<SparseCooBuilder>();

      memcpy(output->MutableValues().MutableData<int64_t>(), values.Data<int64_t>(), nnz * sizeof(int64_t));
      memcpy(builder.GetOrCreate()->MutableIndices().MutableData<int64_t>(), indices.Data<int64_t>(), nnz * numdims * sizeof(int64_t));
  ...
  class OpKernelImpl final : public OpKernel {
   public:
    ...
    Status Compute(OpKernelContext* ctx) const override {
      ...
      memcpy(builder.GetOrCreate()->MutableIndices().MutableData<int64_t>(), input_indices.Data<int64_t>(), input_indices.SizeInBytes());
      return Status::OK();
    }
  };
